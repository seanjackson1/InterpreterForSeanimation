...
public class Tokenizer {
  ...
  public String next() {
    ...
    } else if (text.substring(index).length() > 1 && (text.substring(index, index + 2).equals("!=") || text
        .substring(index, index + 2).equals("=="))) {

      token = text.substring(index, index + 2);
      index += 2;

    } else {
      // not alphanumeric. therefore, this symbol is assumed to be a one character
      // token.
      token = text.substring(index, index + 1);
      index++;
    }
